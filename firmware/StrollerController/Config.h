...
/**
 * Mid-FTB (DIP/FADE): ignore active-rule matches so trailing ads / UART drain cannot
 * restart the BLE look. Queue is also flushed on ON→DIP.
 * Black hold (COOLDOWN, onMatch): allow re-apply only after this quiet gap since the
 * last packet — intentional new cast, not the burst that just ended. Fixed cooldown
 * mode never re-applies during black hold.
 */
