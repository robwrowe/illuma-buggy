...
export default function ZonesScreen() {
  ...
  const addDrawPointAt = useCallback((coord: LatLng) => {
    ...
    if (insertModeRef.current && drawPointsRef.current.length >= 2) {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        const d =
          Math.pow(coord.latitude - midLat, 2) +
          Math.pow(coord.longitude - midLng, 2);
        if (d < bestDist) {
          bestDist = d;
          bestIdx = i;
        }
  ...
  const commitPresetZone = () => {
    if (!newZoneName.trim()) {
      Alert.alert("Required", "Enter a zone name.");
      return;
    }
    const overlapping = zones.find((z) =>
      polygonsOverlap(drawPoints, z.polygon),
    );
    const save = () => {
      addZone({
        id: generateId(),
  ...
  const saveEdit = () => {
    ...
    if (editingIndoor) {
      updateIndoorZone(editingIndoor.id, editingIndoor);
      saveToStorage();
    }
  ...
  const deleteEdit = () => {
    if (editingZone) {
      removeZone(editingZone.id);
      saveToStorage();
    }
    if (editingIndoor) {
      removeIndoorZone(editingIndoor.id);
      saveToStorage();
    }
    setShowEditModal(false);
    setEditingZone(null);
  ...
  return (
    <View style={s.container}>
              ...
              onPress={(e) => {
                if (drawModeRef.current !== "none") {
                  if (e?.nativeEvent?.coordinate)
                    addDrawPointAt(e.nativeEvent.coordinate);
                  return;
                }
                openZoneEdit(zone);
              }}
      ...
      {/* Active zones banner — above toolbar so map location control stays tappable */}
      {activeZoneIds.length > 0 && !isDrawing && (
        <View style={s.activeBanner} pointerEvents="box-none">
          <View style={s.activeBannerInner}>
            <Text style={s.activeBannerTitle}>
              📍 In zone{activeZoneIds.length > 1 ? "s" : ""}:
            </Text>
      ...
      {selectedPinIdx !== null && (
        <View style={s.pinHint}>
          <Text style={s.pinHintText}>
            Pin {selectedPinIdx + 1} selected — tap anywhere on map to move it
          </Text>
        </View>
      )}
      ...
      {!isDrawing ? (
        <View style={s.toolbar}>
          <TouchableOpacity
            style={s.toolBtn}
            onPress={() => setDrawMode("preset")}
          >
            <Text style={s.toolBtnText}>＋ Preset Zone</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.toolBtn, { borderColor: INDOOR_COLOR }]}
            onPress={() => setDrawMode("indoor")}
          >
            <Text style={[s.toolBtnText, { color: INDOOR_COLOR }]}>
              ＋ Indoor Zone
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
        ...
        <View style={s.toolbar}>
          <View style={{ flex: 1 }}>
            ...
            {selectedPinIdx !== null && (
              <Text style={[s.drawHint, { fontSize: 10, opacity: 0.7 }]}>
                Tap pin again to deselect
              </Text>
            )}
          </View>
          {drawPoints.length >= 3 && (
            ...
            >
              <Text
                style={[s.toolBtnText, insertMode && { color: colors.warning }]}
              >
                ⊕
              </Text>
            </TouchableOpacity>
          ...
          {drawPoints.length > 0 && (
            <TouchableOpacity
              style={[s.toolBtn, { paddingHorizontal: 10 }]}
              onPress={undoLastPin}
            >
          ...
          <TouchableOpacity
            style={[s.toolBtn, { borderColor: colors.danger }]}
          ...
          <View style={s.modal}>
            ...
            {drawMode === "preset" && (
              <>
                ...
                <Text style={s.hint}>
                  Leave as boundary only for show locations or park grouping —
                  no effect on enter.
                </Text>
                <ScrollView style={{ maxHeight: 160 }}>
                  ...
                  >
                    <Text
                      style={[
                        s.optionText,
                        !newZonePreset && { color: colors.primary },
                      ]}
                    >
                      None — boundary only
                    </Text>
                  ...
                  {presets.map((p) => (
                    <TouchableOpacity
                      ...
                      style={[
                        s.option,
                        newZonePreset === p.id && s.optionActive,
                      ]}
            ...
            <View style={s.modalRow}>
              ...
              <TouchableOpacity
                style={s.saveBtn}
                onPress={
                  drawMode === "preset" ? commitPresetZone : commitIndoorZone
                }
              >
      ...
      <Modal visible={showEditModal} transparent animationType="slide">
        <View style={s.overlay}>
          <View style={s.modal}>
            <Text style={s.modalTitle}>
              Edit {editingZone ? "Preset Zone" : "Indoor Zone"}
            </Text>
            {editingZone && (
              <>
                <Text style={s.fieldLabel}>Name</Text>
                <TextInput
                  ...
                  onChangeText={(v) =>
                    setEditingZone({ ...editingZone, name: v })
                  }
                  placeholderTextColor={colors.textMuted}
                />
                <Text style={s.fieldLabel}>Preset (optional)</Text>
                <ScrollView style={{ maxHeight: 160 }}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setEditingZone({ ...editingZone, presetId: "" })
                    }
                  >
                    <Text
                      style={[
                        s.optionText,
                        !editingZone.presetId && { color: colors.primary },
                      ]}
                    >
                      None — boundary only
                    </Text>
                  ...
                  {presets.map((p) => (
                    <TouchableOpacity
                      ...
                      style={[
                        s.option,
                        editingZone.presetId === p.id && s.optionActive,
                      ]}
                      onPress={() =>
                        setEditingZone({ ...editingZone, presetId: p.id })
                      }
                    >
                      <Text
                        style={[
                          s.optionText,
                          editingZone.presetId === p.id && {
                ...
                <View style={s.switchRow}>
                  ...
                  <Switch
                    value={editingZone.enabled}
                    onValueChange={(v) =>
                      setEditingZone({ ...editingZone, enabled: v })
                    }
                    trackColor={{
                  ...
                  <>
                    ...
                    <ScrollView style={{ maxHeight: 120 }}>
                      <TouchableOpacity
                        style={[
                          s.option,
                          !editingZone.parkId && s.optionActive,
                        ]}
                        onPress={() =>
                          setEditingZone({ ...editingZone, parkId: undefined })
                        }
                      >
                        <Text
                          style={[
                            s.optionText,
                            !editingZone.parkId && { color: colors.primary },
                          ]}
                        >
                          Ungrouped
                        </Text>
                      ...
                      {parks.map((p) => (
                        <TouchableOpacity
                          ...
                          style={[
                            s.option,
                            editingZone.parkId === p.id && s.optionActive,
                          ]}
                          onPress={() =>
                            setEditingZone({ ...editingZone, parkId: p.id })
                          }
                        >
                          <Text
                            style={[
                              s.optionText,
                              editingZone.parkId === p.id && {
                                color: colors.primary,
                              },
                            ]}
                          >
            ...
            {editingIndoor && (
              <>
                ...
                <TextInput
                  style={s.input}
                  value={editingIndoor.name}
                  onChangeText={(v) =>
                    setEditingIndoor({ ...editingIndoor, name: v })
                  }
                  placeholderTextColor={colors.textMuted}
                />
                <View style={s.switchRow}>
                  <Text style={s.fieldLabel}>Enabled</Text>
                  <Switch
                    value={editingIndoor.enabled}
                    onValueChange={(v) =>
                      setEditingIndoor({ ...editingIndoor, enabled: v })
                    }
                    trackColor={{
                      false: colors.borderFocus,
                      true: colors.primary,
                  ...
                  <>
                    ...
                    <ScrollView style={{ maxHeight: 120 }}>
                      <TouchableOpacity
                        style={[
                          s.option,
                          !editingIndoor.parkId && s.optionActive,
                        ]}
                        onPress={() =>
                          setEditingIndoor({
                            ...editingIndoor,
                      ...
                      >
                        <Text
                          style={[
                            s.optionText,
                            !editingIndoor.parkId && { color: colors.primary },
                          ]}
                        >
                          Ungrouped
                        </Text>
                      ...
                      {parks.map((p) => (
                        <TouchableOpacity
                          ...
                          style={[
                            s.option,
                            editingIndoor.parkId === p.id && s.optionActive,
                          ]}
                          onPress={() =>
                            setEditingIndoor({ ...editingIndoor, parkId: p.id })
                          }
                        >
                          <Text
                            style={[
                              s.optionText,
                              editingIndoor.parkId === p.id && {
                                color: colors.primary,
                              },
                            ]}
                          >
            ...
            <View style={s.modalRow}>
              <TouchableOpacity
                ...
                onPress={() =>
                  Alert.alert("Delete", "Delete this zone?", [
                    { text: "Cancel", style: "cancel" },
                    {
                      text: "Delete",
                      style: "destructive",
              ...
              >
                <Text style={[s.cancelBtnText, { color: colors.danger }]}>
                  Delete
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
            ...
            <View style={s.modalRow}>
              {(["preset", "indoor"] as const).map((m) => (
                <TouchableOpacity key={m} onPress={() => setListMode(m)}>
                  <Text
                    style={[s.tabLabel, listMode === m && s.tabLabelActive]}
                  >
                ...
                <View style={s.zoneRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    {"presetId" in z && (
                      <Text style={s.hint}>
                        {(z as Zone).presetId
                          ? (presets.find((p) => p.id === (z as Zone).presetId)
                              ?.name ?? "Preset missing")
                          : "Boundary only"}
                      </Text>
                    )}
                  ...
                  <Switch
                    ...
                    onValueChange={(v) => {
                      listMode === "preset"
                        ? updateZone(z.id, { enabled: v })
                        : updateIndoorZone(z.id, { enabled: v });
                      saveToStorage();
                    }}
                    trackColor={{
                      false: colors.borderFocus,
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      listMode === "preset"
                        ? openZoneEdit(z as Zone)
                        : openIndoorEdit(z as IndoorZone);
                      setShowList(false);
                    }}
                  >
                    <Text
