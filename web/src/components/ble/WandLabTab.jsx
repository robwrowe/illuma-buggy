...
export function WandLabTab({ data, update }) {
  ...
  const loadLogEntry = (e) => {
    ...
    if (arr.length) setByteArray(arr, e.presetKey || 'log');
  ...
  return (
    <ScrollArea h="100%" type="auto">
      <Stack p="md" gap="md" maw={900}>
        ...
        <SectionHead>MagicBand+ commands</SectionHead>
        <Text size="xs" c="dimmed">
        ...
        <Group gap="xs" wrap="wrap">
          <Button variant="default" onClick={loadMbCommand} style={{ flex: 1, minWidth: 120 }}>
            Load into bytes
          </Button>
        ...
        <SectionHead>Raw bytes</SectionHead>
        <Field label="Paste hex / capture">
          <WandLabCapturePaste
            hexPaste={hexPaste}
        ...
        <Field label="Starlight / show preset">
          <SearchableSelect
            ...
            allowEmpty
            onChange={loadPreset}
            placeholder={presetKey.startsWith('mb:') ? presetKey : 'Load show preset…'}
            options={Object.keys(SW_FX_PRESET_BYTES).map((k) => ({ value: k, label: k, searchText: k }))}
          />
        </Field>

        <Group gap="xs" align="center">
          <Button onClick={() => sendBytes(bytes)} loading={sending} disabled={bytes.length === 0} style={{ flex: 1 }}>
            Send raw bytes (/send)
          </Button>
