        ...
        <>
          ...
          <Group gap={6} align="center" wrap="wrap" mb="xs">
            ...
            <NumberInput
              ...
              size="xs"
              value={seg.wledSegId}
              onChange={(v) => set({ wledSegId: Math.max(0, parseInt(String(v), 10) || 0) })}
              hideControls
            ...
            <NumberInput
              ...
              size="xs"
              value={seg.start}
              onChange={(v) => set({ start: Math.max(0, parseInt(String(v), 10) || 0) })}
              hideControls
              styles={{ input: { textAlign: 'center', fontFamily: 'monospace' } }}
            />
            <Text size="xs" c="dimmed">
            ...
            <NumberInput
              ...
              size="xs"
              value={seg.stop}
              onChange={(v) => set({ stop: Math.max(0, parseInt(String(v), 10) || 0) })}
              hideControls
              styles={{ input: { textAlign: 'center', fontFamily: 'monospace' } }}
            />
          </Group>
            ...
            {['grp', 'spc', 'of'].map((f) => (
              <Field key={f} label={f}>
                <NumberInput
                  ...
                  onChange={(v) =>
                    set({
                      [f]: f === 'grp' ? Math.max(1, parseInt(String(v), 10) || 1) : parseInt(String(v), 10) || 0,
                    })
                  }
                  hideControls
                  styles={{ input: { textAlign: 'center', fontFamily: 'monospace' } }}
          ...
          <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="xs" mb="xs">
            <Field label="Effect">
              <SearchableSelect
                ...
                onChange={(v) => set({ fx: v === '' ? -1 : parseInt(String(v), 10) })}
                options={fxOpts}
            ...
            <Field label="Palette">
              <SearchableSelect
                ...
                onChange={(v) => set({ pal: v === '' ? -1 : parseInt(String(v), 10) })}
                options={palOpts}
                placeholder="(none)"
                allowEmpty
              />
            ...
            <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="xs" mb="sm">
              ...
              <Field label="LED map">
                <SearchableSelect
                  ...
                  onChange={(v) => updateMap(selected.id, { ledmap: parseInt(String(v), 10) || 0 })}
                  options={LEDMAP_OPTS}
                  allowEmpty={false}
                />
              </Field>
