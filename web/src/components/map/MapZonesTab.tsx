...
export function MapZonesTab({ data, update, mapsReady }) {
  ...
  const zonePoly = useRef<any[]>([]);
  const pinMarkers = useRef<any[]>([]);
  const previewPoly = useRef<any>(null);
  const editPinMarkers = useRef<any[]>([]);
  const editPreviewPoly = useRef<any>(null);
  ...
  const [collapsedParkSections, setCollapsedParkSections] = useState<Record<string, any>>({});
  const [editZone, setEditZone] = useState<any>(null);
  const [editIndoor, setEditIndoor] = useState<any>(null);
  const [mapType, setMapType] = useState('satellite');
  const [locationSearch, setLocationSearch] = useState('');
  const [searching, setSearching] = useState(false);
  const [mapReady, setMapReady] = useState(false);
  const [highlightZoneId, setHighlightZoneId] = useState<any>(null);
  ...
  const commitZone = () => {
    ...
    if (drawMode === 'preset') {
      const zone: any = { id: generateId(), name: formName, polygon: normalizePolygon(drawPoints), presetId: formPreset || '', enabled: true };
      if (formParkId) zone.parkId = formParkId;
      update({ zones: [...data.zones, zone] });
    } else {
      const zone: any = { id: generateId(), name: formName, polygon: normalizePolygon(drawPoints), enabled: true };
      if (formParkId) zone.parkId = formParkId;
