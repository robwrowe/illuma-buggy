...
import type { AppData, ColorCalibration } from '../types/app';
...
export function createEmptyColorCalibration(overrides: Partial<ColorCalibration> = {}): ColorCalibration {
  return {
    enabled: false,
    curves: {
...
export const DEFAULT_DATA: AppData = {
  ...
  overrideKillOnZone: false, presetApplyMode: 'legacy', bleEffectTransitionMs: 700,
...
export function buildRecallPayload(preset, recall, segmentMaps) {
  ...
  const payload: any = { on: true, ledmap: resolvePresetLedmap(preset, segmentMaps) };
