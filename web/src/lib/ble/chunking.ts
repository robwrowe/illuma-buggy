...
export class WebBleBoard {
  ...
  sendRunning: boolean;
  sendQueue: any[];
  connListeners: Set<(connected: boolean) => void>;
  msgListeners: Set<(msg: any) => void>;
  _chunkFailListeners: Set<(info: any) => void>;
  chunkBuffer: Record<string, any>;
  chunkNextSeq: Record<string, any>;
   ...
   */
  async requestRuleLog({ limit = 50, events = null, timeoutMs = 20000 } = {}): Promise<{ meta: any; lines: any[] }> {
    if (!this.cmdChar) throw new Error('Not connected to IllumaBuggy');
    const payload: any = { type: 'get_rule_log', limit };
