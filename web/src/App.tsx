...
import type { AppData, AppDataUpdate } from './types/app';

type ProfileRecord = AppData & { savedAt?: string };
...
export function App() {
  ...
  const update = useCallback<AppDataUpdate>((patch) => {
    setData((prev) => {
  ...
  const importJSON = (file?: File | null) => {
    ...
    reader.onload = (ev) => {
      try {
        replaceData(migrateConfig(JSON.parse(String(ev.target?.result || ''))));
        alert('Imported!');
      } catch {
        alert('Invalid file');
      }
